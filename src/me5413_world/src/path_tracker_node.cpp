...
PathTrackerNode::PathTrackerNode() : tf2_listener_(tf2_buffer_)
{
  ...
  this->pub_cmd_goal_ = nh_.advertise<geometry_msgs::PoseStamped>("/jackal_velocity_controller/cmd_goal", 1);
...
void PathTrackerNode::localPathCallback(const nav_msgs::Path::ConstPtr& path)
{
  ...
  int total_poses = path->poses.size();

  this->pose_world_goal_ = path->poses[total_poses-10].pose;
  geometry_msgs::PoseStamped pose_goal;
  pose_goal.header.frame_id = this->world_frame_;
  pose_goal.pose = this->pose_world_goal_;
  this->pub_cmd_goal_.publish(pose_goal);
